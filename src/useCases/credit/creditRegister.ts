...
export class UseCaseCreditRegister {
  ...
  async Register(
    ...
    try {
      ...
      if (credit.interestType === "Simple") {
        Itotal =
          parseFloat(credit.amountApproved) * intRate * credit.quotesNumber
      }

      if (credit.interestType === "Compuesto") {
        Itotal =
          parseFloat(credit.amountApproved) *
            Math.pow(1 + intRate, credit.quotesNumber) -
          parseFloat(credit.amountApproved)
      }

      const totalC =
        parseFloat(credit.amountApproved) + parseFloat(Itotal.toString())
      ...
      for (let i = 0; i < credit.quotesNumber; i++) {
        ...
        if (newCredit.interestType === "Simple") {
          cInterest =
            parseFloat(newCredit.totalInterest.toString()) / credit.quotesNumber
          cCapital = parseFloat(credit.amountApproved) / credit.quotesNumber
          total = cCapital + cInterest
        }
        if (newCredit.interestType === "Compuesto") {
          const quotes = this.calculateQuotesCompoundInterest(
            newCredit.amountApproved,
            newCredit.interestRate,
            newCredit.quotesNumber,
          )
      ...
      const newAmount =
        parseFloat(newCredit.amountApproved.toString()) +
        parseFloat(userFound.balance.toString())

      const updateBalance = userRepository.updateBalance(
        credit.userId,
        newAmount,
      )
      ...
      await EmailService.sendEmailCreditApproved({
        email: userFound.email,
        name: userFound.name,
        amount: newCredit.amountApproved.toString(),
      })
  ...
  calculateQuotesCompoundInterest(
    capital: number,
    interesRate: number,
    quotaNumber: number,
  ) {
    let quoteFixed =
      (capital * interesRate * Math.pow(1 + interesRate, quotaNumber)) /
      (Math.pow(1 + interesRate, quotaNumber) - 1)
